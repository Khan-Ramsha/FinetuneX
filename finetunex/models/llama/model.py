...
class LlamaDecoderBlock(nn.Module):
    def __init__(self, config: Config, layer_idx: int, args: SFTConfig):
        super().__init__()
        self.input_layernorm = RMSNorm(config.hidden_size, config.rms_norm_eps)
        self.attn = LlamaGroupQueryAttention(config.hidden_size, config.num_attention_heads, config.num_key_value_heads, layer_idx = layer_idx, use_flashattn = args.use_flash_attn)
...
class LlamaModel(BaseModel):
    ...
    def _causal_mask(self, T, device):
        if (not hasattr(self, "_causal_mask_cache") or self._causal_mask_cache.size(-1) < T):
    ...
    def decoder_layer(self, config, layer_idx):
        return LlamaDecoderBlock(config, layer_idx, args = self.args)
    ...
    def forward(self, input_ids, attn_mask = None,labels = None):
        ...
        # next four lines are for whole masked rows when using left padding 
        all_masked = combined_mask.all(dim=-1, keepdim=True)         # [B, 1, T, 1] — True where row is fully hidden
        unhide_first = torch.zeros_like(combined_mask)
        unhide_first[..., 0] = all_masked.squeeze(-1)
