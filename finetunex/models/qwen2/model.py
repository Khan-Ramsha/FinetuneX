...
class DecoderBlock(nn.Module):
    def __init__(self, config: Config, layer_idx : int, args: SFTConfig):
        ...
        self.attn = GroupQueryAttention(config.hidden_size, config.num_attention_heads, config.num_key_value_heads, layer_idx = layer_idx, use_flashattn = args.use_flash_attn)
...
class Qwen2Model(BaseModel):
    ...
    def decoder_layer(self, config, layer_idx):
        return DecoderBlock(config, layer_idx, args=self.args)
