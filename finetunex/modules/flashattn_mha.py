...
def _attn_fwd(
    ...
    """
    tl.program_id(0) -> unique ids responsible to handle/workon DIFFERENT blocks of , each id get different block to work on.
    suppose-> program_id = 0, works on 1st block [0,1,2,3.....63]  [0:BLOCKSIZE]
              program_id = 1, works on 2nd block = [64,.....127] [BLOCKSIZE:2*BLOCKSIZE]
    num_warps in autotuning --> tells about the number of threads within a block(for per program), say num_warps = 2 then 2*32 = 64 threads, for 4 = 4 * 32 = 128 threads.
    """
    ...
    #qvk_offset is required to go to a particular batch & head, batchidx * stride batch -> batch to skip, idxhead*stride head -> skips the head within the batch
    #BLOCK OF QUERY IS FIXED
    Q_block_ptr = tl.make_block_ptr(
        ...
        shape = (SEQ_LEN, HEAD_DIM), # can say the whole block (a total block that has many blocks-the full sequence) you are accessing the elements from
    ...
    """
    order parameter describes:
    the memory layout- dim 1 (headdim) changes faster (so DO PRIORITIZE IT) 
    & dim0 changes slower (that is you need to process/go through a head dim for a sequence right which would be slow than travelling headdim which has stride of 1)
    """
    ...
    ## we use offset (0,0) for V & K block which gets udpated using advance()
    ...
    """
    if blockidx  = 1, blocksize = 64 then 1 * 64 + (0, 1, 2, ...... 63) = [64, 65, 66 67, ....]
    """
...
def _attn_bwd_dk_dv(
    ...
    """
    below if else block determines the loop range,
    For Stage==3, 
    lo = 64 (assuming we are at second block)
    hi = seqlen
    num_steps = (1024 - 64) = 960//64 = 15 blocks to be processed
    curr_q = lo (why lo? because you can't look at KV > Q so Q has to be greater or equal to KV in order to attend that's the reason we dont set it 0)
    But in non-causal case you need to go through all blocks so it starts with curr_q = 0
    """
    if STAGE == 3:  # Causal
...
class TritonAttn(torch.autograd.Function):
    ...
    def forward(ctx, Q, K, V, causal, softmax_scale):
        ...
        O = torch.empty_like(Q) # allocate the memory to the output
        flag = 3 if causal else 1
        #3D grid
        grid = lambda args: (
            # (seqlen/blocksize) = total number of blocks of Q
            #cdiv is ceil div
            triton.cdiv(seq_len, args["BLOCK_SIZE_Q"]), #which group of queries are we going to work with. (axis = 0)
            batch_size * num_heads, #which head of which batch element are we going to work with (axis = 1)
            1 #(axis = 2)
        )
        #number of parallel programs: (Batch_size * num_heads * num_blocks_q)
        """
        for example: batchsize = 2, num_heads = 8, seqlen = 512, blocksize_q = 64
        number of Q blocks = 512/64 = 8 Q blocks
        grid = (8, 16, 1) = 128 programs running in parallel!!
        now each program handles 64 queries (1 block)

        let's get deep into what grid is?
        Grid tells how many programs will run in parallel across GPU
        In cuda:  (programmer specify) -> how many blocks we want & how many threads we want for each block
        In triton: (programmer specify) -> how many blocks we want. Triton chooses how many threads to launch 
        In triton kernels, we write code for "BLOCK" of data. Programming in triton happens at BLOCK LEVEL. 

        axis 0: parallelism over sequence - launching enough programs that covers every block (one program per block)
        axis 1: parallelism over data (batch & head) 
        axis = 2: not using this, as we dont need the third dim of parallelism. since it is unused it is set to 1.

        """
        M = torch.empty(
            (batch_size, num_heads, seq_len), device = Q.device, dtype = torch.float32
        )
        #passing this 3d grid to kernel & above we write the kernel. 
        _attn_fwd[grid](
            Q = Q,
            K = K, 
            V = V,
